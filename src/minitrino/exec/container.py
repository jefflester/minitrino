...
from minitrino.docker.wrappers import MinitrinoContainer
from minitrino.errors import MinitrinoError
from minitrino.exec.result import CommandResult
from minitrino.exec.utils import detect_container_shell
...
class ContainerCommandExecutor:
    ...
    @property
    def _api_client(self) -> docker.APIClient:
        """Return the API client, asserting it is initialized."""
        assert self._ctx.api_client is not None, "Docker API client is not initialized"
        return self._ctx.api_client
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        container = cast(MinitrinoContainer, kwargs.get("container"))
        if not container:
            raise ValueError(
        ...
        docker_cmd: list[str] = [self.shell, "-c", " ".join(command)]
        ...
        try:
            exec_handler = self._api_client.exec_create(
                container.name,
                cmd=docker_cmd,
            ...
            output_generator: Generator = self._api_client.exec_start(
            ...
            for line in self._buffer_exec_output_lines(
                ...
                output += line
            rc = self._api_client.exec_inspect(exec_handler["Id"]).get("ExitCode")
            if rc in [126, 127]:
    ...
    def stream_execute_with_result(
        ...
        **kwargs : Any
        ...
        exec_id_holder: dict[str, str] = {"exec_id": ""}
            ...
            def monitor_exec() -> None:
                ...
                try:
                    ...
                    while not completion_event.is_set():
                        try:
                            exec_info = self._api_client.exec_inspect(
