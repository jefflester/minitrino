...
- 🎉 **Trino Support**: Full compatibility with Trino 443+ alongside Starburst
  443-e+
- 🚀 **Multi-Cluster**: Deploy and manage multiple isolated clusters
  simultaneously
- 🏗️ **Modernized Architecture**: Complete overhaul with Python 3.10+,
  pyproject.toml, and enhanced modularity
- 📊 **Production Logging**: Advanced logging system with spinners, buffering,
  and colored output
- 📚 **Comprehensive Docs**: Full Sphinx documentation site with API references
  and user guides
- 🧪 **Testing Overhaul**: Enhanced test infrastructure with better coverage and
  organization
...
⚠️ **Important**: This release contains breaking changes. Please review this
...
- **setup.py → pyproject.toml**: Modern Python packaging standards
- **Command files renamed**: `cmd_*.py` → `*.py` (e.g., `cmd_provision.py` →
  `provision.py`)
...
1. **Update configuration file** (`~/.minitrino/minitrino.cfg`) with new
   variables
1. **Install 3.0.0**: `pip install --upgrade minitrino`
1. **Install library**: `minitrino lib-install` (auto-prompted if needed)
1. **Test with new syntax**: `minitrino -c test provision` to verify
1. **Update custom modules** if you have any
...
- **Independent lifecycles**: Each cluster operates independently with its own
  resources
- **Example**: `minitrino -c prod provision --modules hive,iceberg` + `minitrino -c dev provision --modules postgres`
...
- **Default version**: Updated to 479 (Trino)
...
- **Restructured test package**: `test/` → `tests/` with new subdirectory
  organization:
...
- **Expanded pre-commit hooks**: shellcheck, yamllint, auto-generated
  docs
