...
from minitrino.logging import levels, sink
...
class TestMinitrinoLogger:
    ...
    @patch("minitrino.logging.logger.prompt")
    @patch("minitrino.logging.logger.style")
    ...
    @patch("minitrino.logging.utils.get_caller_fq_name")
...
class TestSpinnerAndLogBuffer:
    """Tests for Spinner context manager and LogBuffer."""

    def test_log_buffer_buffer_and_flush(self, capsys) -> None:
        """Test LogBuffer buffers messages and flushes them to the correct stream."""
        from minitrino.logging.spinner import LogBuffer
    ...
    def test_spinner_buffer_and_flush(self, monkeypatch, capsys) -> None:
        """Test Spinner buffers logs and flushes after context exit.

        In CI/headless runs spinner artifacts may clear terminal output, so we only
        assert no exceptions are raised and the code path is covered. LogBuffer
        correctness is tested in test_log_buffer_buffer_and_flush.
        """
        import sys

        from minitrino.logging import sink as lg_sink
        from minitrino.logging import spinner as lg_spinner

        logger = MinitrinoLogger("minitrino-spin")
        test_sink = lg_sink.SinkCollector()
        logger._log_sink = test_sink
        handler = lg_sink.SinkOnlyHandler(test_sink, logging.Formatter("%(message)s"))
        ...
        spinner = lg_spinner.Spinner(logger, test_sink, always_verbose=False)  # type: ignore[arg-type]
    ...
    def test_spinner_no_buffer_if_always_verbose(self, monkeypatch, capsys) -> None:
        """Test Spinner disables buffering in always_verbose mode.

        In CI/headless runs output may not be captured due to TTY mechanics. We only
        assert no exceptions are raised and the code path is covered.
        """
        import sys
        ...
        logger = MinitrinoLogger("minitrino-spin2")
        test_sink = lg_sink.SinkCollector()
        logger._log_sink = test_sink
        spinner = lg_spinner.Spinner(logger, test_sink, always_verbose=True)  # type: ignore[arg-type]
        monkeypatch.setattr(sys.stdout, "isatty", lambda: True)
        with spinner.spinner("spin message"):
