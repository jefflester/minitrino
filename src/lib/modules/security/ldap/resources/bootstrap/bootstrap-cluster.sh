...
LDAP_URI=$(cat /etc/"${CLUSTER_DIST}"/password-authenticator.properties | grep "ldaps" | sed -r "s/^.*ldaps:\/\/(.+:[0-9]+).*$/\1/")
...
ls /etc/"${CLUSTER_DIST}"/ldap-users/*.ldif | while read -r LDIF_FILE;
do
